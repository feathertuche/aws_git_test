...
class Command(BaseCommand):
    ...
    def process_invoices(self, payload):
        ...
        Returns:
            return_type: None
        """
        api_log(msg="*********** Calling PROCESS INVOICE bloc to execute NEW invoices ***********")
    ...
    def retry_invoices_from_api(self, payload: list):
        ...
        Returns:
            return_type: None
        """
        api_log(msg="*********** Calling retry bloc executing invoices matching in DB ***********")
        from INVOICES.models import InvoiceAttachmentLogs
        from INVOICES.models import CronRetry

        # Read the table and fetch all rows
        read_invoice_id = CronRetry.objects.filter(cron_execution_time__lte=datetime.now())
        invoices = []
        pattern = r"'problem_type': '([^']+)'"
        ...
        try:
            for invoice_payload in payload:
                for invoice_ids in read_invoice_id:
                    if payload["model"]["kloo_invoice_id"] == invoice_ids.kloo_invoice_id:
                        response = self.create_invoice(invoice_payload)
                        if response.status_code == status.HTTP_201_OK:
                            api_log(msg=f"Response from create invoice in RETRY INVOICES method :=> {response}")
                            api_log(
                                msg=f"Response DATA from create invoice in RETRY INVOICES method :=> {response.data}")
                            ...
                            if status_code:
                                if status_code in [404, 400]:
                                    ...
                                    api_log(
                                        msg=f"Response in the retry_invoices_from_api function in management command "
                                            f"file::: {response} and "
                                            f"Invoice_id is {invoice_id}")
